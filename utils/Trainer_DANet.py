...
import torch
from models import UNet, fcn, Discriminator, UNet_withDA
...
class Trainer_DANet(object):
    ...
    def fit(self):
        ...
        loss_filename = os.path.join(self.config.checkpoint, 'loss_by_DANet.csv')
        loss_df.to_csv(loss_filename, index=False, float_format='%.4f')
        eval_df = pd.DataFrame(self.eval_history)
        eval_filename = os.path.join(self.config.checkpoint, 'eval_0.5_by_DANet.csv')
